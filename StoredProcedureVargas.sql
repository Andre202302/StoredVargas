...
-- FNCTIONS
-- 1.Funcion para saber la capacidad de pago del asociado, tomando su salario dividido 2
DELIMITER $$
CREATE FUNCTION capacidad_pago (n_id INT)
RETURNS DECIMAL(11,2)
READS SQL DATA
BEGIN
	DECLARE capacidad DECIMAL(11,2);
    SET capacidad = (SELECT salario FROM asociados WHERE id=n_id)/2;  
    RETURN capacidad;
END$$
DELIMITER ;

-- 2.Funcion para calcular el valor pagado de los creditos, se debe ingresar el codigo del credito para obtener el resultado
DELIMITER $$
CREATE FUNCTION valor_pagado (n_id_cred INT)
RETURNS INT
DETERMINISTIC
BEGIN
	DECLARE pagado INT;
    SET pagado = (SELECT sum(valor_credito-saldo_credito) FROM credito WHERE cod_credito = n_id_cred);    
    RETURN pagado;
END$$
DELIMITER ;

-- STORED PROCEDURE
-- 1.SP que devuelve el resultado de la creación de una cidudad
DELIMITER $$		
CREATE PROCEDURE insertar_ciudad(IN p_cod_ciudad VARCHAR(5), -- sugerencia para crear codigo 51002 
			                     IN p_nombre_ciudad VARCHAR(50), -- sugerencia para crearnombre MEDELLIN
                                 IN p_cod_pais VARCHAR(3)  --  Sugerencia para crear pais 169
            )
BEGIN
	 IF ((SELECT cod_ciudad FROM ciudad WHERE cod_ciudad = p_cod_ciudad AND cod_pais = p_cod_pais) IS NOT NULL ) THEN
	 SELECT 'ERROR: La ciudad ya existe creada en ese pais' AS Error;
     ELSE
     INSERT INTO ciudad (cod_ciudad, n_ciudad, cod_pais) 
     VALUES (p_cod_ciudad, p_nombre_ciudad, p_cod_pais);	
     SELECT * FROM ciudad WHERE cod_ciudad = p_cod_ciudad;		
    END IF;		
END $$
DELIMITER ; 

-- 2.SP permite establecer el campo y ordenamiento ascedente o descendente de la tabla de tercero 
DELIMITER $$		
CREATE PROCEDURE ordenamiento(IN p_campo VARCHAR(20), -- campo sugerido para ordenar por apellido N_APELL1
							  IN p_asc_desc VARCHAR(50)
							  )
BEGIN
	IF p_asc_desc <> '' THEN	
		SET @_order = CONCAT('ORDER BY',' ',p_campo,' ',p_asc_desc);
	ELSE	
		SET @_order = '';
    END IF;		
	SET @laquery = CONCAT('SELECT * FROM workshopvargas.cliente',' ', @_order);	
	
	PREPARE mi_select FROM @laquery;	
    EXECUTE mi_select;		
